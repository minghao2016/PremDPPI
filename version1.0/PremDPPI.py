                ...
                if pdb.split(".")[1] != "pdb":
                    ...
                    for linepdb in fpdb:
                        if linepdb[0:5] == "MODEL":
                            ...
                            continue
                        if ST_play:
                            if linepdb[:4] == "ATOM" and linepdb[21] in usedchains and linepdb[17:20].strip() in normal_format_pro:  # and linepdb[12:16].strip()[0] != 'H':
                                ffpdb.write("%s                  %s\n" % (
    ...
    for line in f:
        ...
        for row in pdbfile_del_unknown:     
            if row[17:26] == last and row == final_row:            # when read final row，append it if equal to last
                above.append(row)
                atoms = [i[13:16].strip() for i in above]          # ！！！16位可能是A，B表示状态， 不能取
...
# 1IGY_A_1.pdb, 1IGY_B_1.pdb, 1IGY_C_1.pdb and 1IGY_D_1.pdb will be produced by 1n8o_p.pdb
def splitchain():
    pdball = []
    f = open(in_file, 'r')
    next(f)
...
# produce 2FTL_I_1.seq, 2FTL_I_1.var, 2FTL_CH1.pdb and 2FTL_CH2.pdb by 2FTL_E_1.pdb and 2FTL_I_1.pdb.
def CleanPdb():
...
# delete 2FTL.pdb and produce renumbered and processed 2FTL.pdb wild-type pdb structure.
def wtpdb():
    pdball = []
...
# produce 2FTL_vmd.psf and 2FTL_vmd.pdb using 2FTL_CH1.pdb and 2FTL_CH2.pdb.
def vmd_wt():
    pdball = []
    f = open(in_file + ".cleaned", 'r')
    _unused = next(f)
...
# produce 2ftl_ch1.psf/crd,2ftl_ch2.psf/crd,2ftl_charmm.psf/crd/pdb,2ftl_namd.psf/pdb. 
def charmmfile_wt():
    pdball = []
    f = open(in_file + ".cleaned", 'r')
    _unused = next(f)
...
# produce 2ftl_namd_restrain.pdb file.
def runrest():
    pdball = []
    f = open(in_file + ".cleaned", 'r')
    _unused = next(f)
...
# produce surface_2ftl_e_1.out and so on. need 2FTL.dssp from crystal structure file.
def surfacefile():
    f = open(in_file + ".cleaned", 'r')
    next(f)
...
def iPot():
    ...
    pdb = f[0].split('\t')[0].split('.')[0].lower()  # 1A43
    # wile type
    wt_res = []
